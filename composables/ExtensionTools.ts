export default class ExtensionTools {
  ...
  public static datetime(date: any, options = { year: 'numeric', month: 'long', day: 'numeric' }) {
    if (date instanceof Date)
      return new Intl.DateTimeFormat(useRuntimeConfig().public.APP_LOCALE, options).format(date);
    date = new Date(date)
    return new Intl.DateTimeFormat(useRuntimeConfig().public.APP_LOCALE, options).format(date);
