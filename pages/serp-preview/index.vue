<template>
  <div class="p-2">
    <div class="mt-4 leading-loose">
      <h1 class="font-bold text-lg text-center">فقط عنوان و توضیحات متا خود را وارد کنید تا یک پیش نمایش از وبسایت
        خودتان را در صفحه سرچ گوگل مشاهده کنید:</h1>
    ...
    <div class="searchBox h-[460px] border p-4 inline-block w-2/5  rounded-[10px] ">
      <div class="searchTitle mb-4">
        ...
        <div class="flex justify-between gap-4">
          <input @input="txtUrl=$event.target.value" placeholder="example:www.segmento.ir"
                 @keyup="minimizeParams($event.target.value)" type="text"
                 class="w-4/5 h-11 text-black placeholder:text-base-400 border border-base-400 rounded-[5px] ltr"/>
          <button class="btn-primary rounded-lg text-sm w-1/5" :disabled="txtUrl.length===0" @click="checkHttpRegex()">
            دریافت
          </button>
        </div>
      </div>
      <div class="title mb-4">
        ...
        <input @input="titleLength=$event.target.value" @keydown="progressBar(titleLength.length,`title`)"
      ...
      <div class="Desc mb-4">
        ...
        <textarea @input="descTitle=$event.target.value" @keydown="progressBar(descTitle.length,`desc`)"
      ...
      <div class="keyword mb-4">
        ...
        <input placeholder="درج کلمه کلیدی (جداشونده با ویرگول و یا فاصله)" type="text"
               class="w-full resize-none text-black placeholder:text-base-400 border border-base-400 rounded-[5px] text-right"/>
      </div>
    </div>
