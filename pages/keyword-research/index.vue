<template>
  <div class="flex flex-col w-full h-full">
    ...
    <!-- page content -->
    <div class="flex flex-col gap-4 p-2 w-full pt-8" :class="data === null ? 'h-full' : 'h-fit'">
      <!-- search box -->
      <form @submit.prevent="search_keywords_request()" class="flex flex-row items-center w-full gap-2">
        <div class="flex flex-row items-center w-full justify-between gap-2 h-10">
          ...
          <div class="flex flex-row items-center justify-center w-[30%]">
            <select class="w-full rounded-[3px] border border-base-400 px-2 py-2" v-model="form.lang">
      ...
      <div class="flex flex-row justify-between w-full gap-4" :class="data === null ? 'h-full' : 'h-fit'">
        ...
        <div class="flex flex-col rounded-[3px] border border-base-400"
          ...
          <div class="flex flex-col items-center justify-center h-full gap-3" v-if="data === null">
          ...
          <div class="flex flex-col pl-2" v-else>
            ...
            <div v-for="(item, index) in data" :key="index" :class="item.length <= 0 ? 'hidden' : ''"
              class="flex flex-col px-2 text-sm">
              <!-- header -->
              <div class="flex flex-row w-full items-center py-2 pr-2">
                <span class="flex justify-center">
                  <span class="w-8 bg-gray-200 text-base-content rounded-[3px] flex justify-center">
                    {{ index }}
                  </span>
                </span>
        ...
        <div class="flex justify-center w-[35%] h-[200vh]" v-if="data !== null">
          <div class="flex flex-col gap-4 items-center w-full">
            ...
            <div class="sticky top-20 left-10 rounded-sm grid grid-cols-8 w-full gap-2">
              <button class="alphabet btn-secondary bg-[#F2F5F770] text-base-content" v-for="(data, index) in cache"
                :class="data.length <= 0 ? 'hidden' : ''" :key="index" :id="`alpha-${index}`"
