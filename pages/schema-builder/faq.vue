<template>
  <div class="w-full h-full">
    ...
    <div class="flex flex-row items-center gap-3 px-2 h-[7%] bg-base-200">
      <TabItem to="/schema-builder/faq" :active="true"> FAQ page </TabItem>
        ...
        <div class="flex flex-col gap-2" v-for="(value, index) in values" :key="value">
          <div class="w-full flex items-center gap-2">
            <div class="w-full flex items-center">
              <InputText class="w-full" :placeholder="`پرسش #${index + 1}`" @input="changeQuestionName(index)"
                v-model="values[index].question" />
            </div>
            <div v-if="values.length > 1" class="w-fit h-fit">
              <button @click="deleteOneQuestion(index)" class="w-[20px] h-[20px] btn-danger-icon">
                ✕
              </button>
            </div>
          </div>
          <div class="w-full h-full align-start">
            <InputTextArea class="h-36 w-full" @input="changeAnswer(index)" placeholder="پاسخ"
