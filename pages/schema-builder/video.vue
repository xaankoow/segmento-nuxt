<template>
  <div class="w-full h-full">
    ...
    <div class="flex flex-row items-center gap-3 px-2 h-[7%] bg-base-200">
      ...
      <TabSeparator></TabSeparator>
      <TabItem to="/schema-builder/video" :active="true"> Video </TabItem>
      <TabSeparator></TabSeparator>
    ...
    <div class="flex justify-start items-start gap-2 w-full h-[93%] p-2">
      ...
      <div id="inputsCard" class="w-1/2 h-full flex flex-col gap-2 align-start justify-start">
        <div class="flex flex-col gap-2">
          <div class="w-full">
            <InputText class="w-full align-start" placeholder="عنوان ویدیو" @input="changeVideoName(0)"
              v-model="valuesVideo.name" />
          </div>
          <div class="w-full h-full align-start">
            <InputTextArea class="h-36 w-full" @input="changeVideoDescription(0)" placeholder="توضیحات ویدیو"
        ...
        <div class="w-full flex flex-row gap-2">
          ...
          <div class="w-2/4 h-10 flex items-center gap-2">
            ...
            <InputDate class="w-full" @change="changeUploadDate" id="uploadDate" v-model="valuesVideo.uploadDate">
            </InputDate>
          </div>
        </div>
        <div v-for="(value, index) in imageNumber" :key="index">
          <div class="w-full flex items-center gap-2">
            <InputURL class="w-full placeholder:text-right" placeholder="آدرس  تصویر" @input="changeimage(index)"
              v-model="valuesVideo.thumbnailUrl[index]" />
            <button @click="deleteOneImage(index)" v-if="imageNumber > 1" class="w-[20px] h-[20px] btn-danger-icon">
              ✕
            </button>
          </div>
        </div>
        ...
        <div class="w-full flex gap-2">
          <div class="w-1/2">
            <InputURL class="w-full placeholder:text-right" placeholder="لینک محتوا" @input="changecontentUrl()"
              v-model="contentUrl" />
          </div>
          <div class="w-1/2">
            <InputURL :dir="embedUrl === '' ? 'rtl' : 'ltr'" class="w-full placeholder:text-right text-left"
              placeholder="آدرس درج embed" @input="changeEmbedUrl()" v-model="embedUrl" />
          </div>
        </div>
        <div>
          <div class="w-full">
            <InputURL class="w-full placeholder:text-right" placeholder="آدرس دقیق میزان پخش ویدئو"
...
function changeVideoMin() {
  ...
  if (videoDuration.value.sec) {
    jsonData.value.duration =
      "PT" + videoDuration.value.min + "M" + videoDuration.value.sec + "S";
  } else {
