<template>
  <div class="w-full h-full">
    ...
    <div class="flex justify-start items-start gap-2 w-full h-[93%] p-2">
      ...
      <div
        id="inputsCard"
        class="w-1/2 h-full flex flex-col gap-2 align-start justify-start"
      >
        <div class="flex flex-col gap-2">
          ...
          <div class="w-full h-full align-start">
            <InputTextArea
              class="h-36 w-full"
              @input="changeVideoDescription(0)"
        ...
        <div class="w-full flex flex-row gap-2">
          <div class="w-1/4 h-10 flex items-center gap-2">
            ...
            <InputNumber
              class="w-full"
              id="min"
              @input="changeVideoMin()"
              v-model="videoDuration.min"
            />
          </div>
          <div class="w-1/4 h-10 flex items-center gap-2">
            ...
            <InputNumber
              class="w-full"
              id="sec"
              @input="changeVideoSec()"
              v-model="videoDuration.sec"
            />
          </div>
          <div class="w-2/4 h-10 flex items-center gap-2">
            ...
            <InputDate
              ...
              id="uploadDate"
              v-model="valuesVideo.uploadDate"
            ></InputDate>
          </div>
        </div>
        <div v-for="(value, index) in imageNumber" :key="index">
          <div class="w-full flex items-center gap-2">
            <InputURL
              ...
              @input="changeimage(index)"
              v-model="valuesVideo.thumbnailUrl[index]"
            />
            <button
              @click="deleteOneImage(index)"
              v-if="imageNumber > 1"
        ...
        <div class="w-full flex gap-2">
          <div class="w-1/2">
            <InputURL
              ...
              @input="changecontentUrl()"
              v-model="contentUrl"
            />
          </div>
          <div class="w-1/2">
            <InputURL
              :dir="embedUrl === '' ? 'rtl' : 'ltr'"
              class="w-full placeholder:text-right text-left"
              placeholder="آدرس درج embed"
              @input="changeEmbedUrl()"
              v-model="embedUrl"
            />
          </div>
        </div>
        <div>
          <div class="w-full">
            <InputURL
              class="w-full placeholder:text-right"
              placeholder="آدرس دقیق میزان پخش ویدئو"
              @input="changeSeekTarget()"
              v-model="seekTargetUrl"
            />
          </div>
        </div>
      </div>
