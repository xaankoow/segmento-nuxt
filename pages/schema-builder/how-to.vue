  ...
  <div class="w-full h-full">
    ...
    <div class="flex justify-start items-start gap-2 w-full h-[93%] p-2">
      ...
      <div class="w-1/2 h-full flex flex-col gap-2 align-start justify-start">
        <!-- description start -->
        <div class="w-full">
          <InputText placeholder="نام" @input="changeDescriptionName()" v-model="valuesDescription.name" class="h-10" />
        </div>
        <div class="w-full h-fit align-start">
          <InputTextArea class="h-32" @input="changeDescriptionDescription()" placeholder="توضیحات"
            v-model="valuesDescription.description" />
        </div>
        <div class="w-full h-10">
          <InputURL placeholder="آدرس تصویر" id="descriptionImageUrl" @input="changeDescriptionImage()"
            v-model="valuesDescription.image" class="text-left placeholder:text-right" dir="ltr" />
        </div>
        <div class="w-full flex gap-2">
          <div class="w-[30%] h-10 flex items-center gap-2">
            <span class="w-40 text-sm">مجموع زمان</span>
            <InputNumber id="totalTime" class="w-full" dir="ltr" @input="changeDescriptionTotalTime()"
          ...
          <div class="w-[30%] h-10 flex items-center gap-2">
            ...
            <InputNumber id="price" class="w-full" dir="ltr" @input="changeCurrencyValue()"
              v-model="valuesCurrency.value" />
          </div>
          <div class="w-2/5 h-10 text-start align-center">
            <DropdownFinalDropDown class="z-index-[1100] h-10">
              ...
              <template v-slot:option>
                <InputRadio v-for="(element, index) in Object.keys(currency)" :key="index"
                  v-model="currency[element].is_checked" @click="changeCurrencyCurrency(element)"
                  :id="`currency_${currency[element].title}`" name="currency">{{ currency[element].title }}</InputRadio>
              </template>
        ...
        <!-- _______________________________________ -->
        <!-- supply start -->
        <div class="w-full flex items-center gap-2 z-[1000]" v-if="supplyNumber" v-for="(value, index) in valuesSupply"
          :key="index">
          <InputText class="w-full" :placeholder="`موجودی ${index + 1}`" @input="changeSupplyName(index)"
            v-model="valuesSupply[index].name" />
          <button @click="deleteOneSupply(index)" class="w-[20px] h-[20px] btn-danger-icon">
            ✕
          </button>
        </div>
        ...
        <!-- _______________________________________ -->
        <!-- tools start -->
        <div class="w-full flex items-center gap-2" v-if="toolsNumber" v-for="(value, index) in valuesTools" :key="index">
          <InputText class="w-full" :placeholder="`ابزار ${index + 1}`" @input="changeToolsName(index)"
            v-model="valuesTools[index].name" />
          <button @click="deleteOneTool(index)" class="w-[20px] h-[20px] btn-danger-icon">
            ✕
          </button>
        </div>
        ...
        <div class="w-full flex flex-col gap-2" v-for="(value, index) in valuesStep" :key="index">
          <div class="w-full flex items-center gap-2">
            <InputText class="w-full align-start" :placeholder="`قدم ${index + 1}`" @input="changeStepText(index)"
              v-model="valuesStep[index].text" />
            <button v-if="valuesStep.length > 1" @click="deleteOneStep(index)" class="w-[20px] h-[20px] btn-danger-icon">
              ✕
            </button>
          </div>
          <div class="w-full">
            <InputURL placeholder="آدرس تصویر" @input="changeStepImage(index)" v-model="valuesStep[index].image"
              class="text-left placeholder:text-right" dir="ltr" />
          </div>
          <div class="w-full flex gap-2">
            <div class="w-full">
              <InputURL placeholder="آدرس" @input="changeStepAddress(index)" v-model="valuesStep[index].address"
                class="text-left placeholder:text-right" dir="ltr" />
            </div>
            <div class="w-[35%]">
              <InputText placeholder="نام" @input="changeStepName(index)" v-model="valuesStep[index].name" />
            </div>
          </div>
        </div>
