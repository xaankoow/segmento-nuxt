    ...
    <div class="w-full h-auto p-5 bg-white">
      <div class="flex items-center justify-between w-full h-[40%]">
        <div
          ...
          <div class="w-full flex items-center">
            ...
            <p class="mr-3">
              {{ config.by_route(`${current_page}/subscription`) }} {{ auth.subscription }}
            </p>
          </div>
        ...
        <div
          ...
          <div class="w-full h-36 px-4 flex flex-col items-center justify-center">
            <ProgressRadial :value="auth.full_time" :remained="auth.remained_days" color="#0A65CD" bg="transparent" size="8rem" fontSize="18px"
              fontSizeText="10px" :textValue="`${config.by_route(`${current_page}/charts/remained-day`)}`"
              textColor="#000000" :sizeCircle="62"></ProgressRadial>
          </div>
      ...
      <div class="flex flex-col w-full h-[92%]">
        ...
        <div class="w-full h-auto flex flex-wrap justify-between items-center">
          <div class="w-[48%] h-[111px] py-3 flex border border-base-400 rounded mt-[38px]" v-for="limit in limits"
            :class="config.by_route(`${current_page}/limits-list/${limit.feature}`) === 'undefined' ? 'hidden' : ''">
            <div class="w-[70%]">
              <div class="flex text-sm items-center">
                <div class="w-[2px] h-5 rounded-[1px] bg-primary ml-3"></div>
                <p>{{ config.by_route(`${current_page}/limits-list/${limit.feature}`) }}</p>
              </div>
            ...
            <div class="w-[30%] h-full flex items-center justify-center pt-4">
              <ProgressRadial :value="limit.budget" :remained="limit.remained" color="#10CCAE" bg="transparent"
                size="5.2rem" fontSize="10px" fontSizeText="8px"
                :textValue="`${config.by_route(`${current_page}/charts/remained-word`)}`" textColor="#000000"
                :sizeCircle="40">
              </ProgressRadial>
            </div>
          </div>
