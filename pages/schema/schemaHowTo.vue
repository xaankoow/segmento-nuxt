    ...
    <div class="w-full h-auto rounded">
        ...
        <div class="flex justify-start items-start gap-4 w-full h-full p-7" >
            ...
            <div class="w-1/2 h-full flex flex-col gap-10 align-start justify-start">
                <div class="w-full " >
                    <InputText placeholder="متن نمونه" @keyup="changeQuestionName(0)" v-model="values[0].question" />
                </div>
                <div class="w-full h-full align-start" >
                    <InputTextArea class="h-36" @keyup="changeAnswer(0)" placeholder="متن نمونه" v-model="values[0].answer" />
                </div>
                <div class="w-full flex gap-3">
                    <div class="w-[40%] h-[45px] flex items-center gap-6">
                        <InputNumber labelValue="مجموع زمان" id="totalTime" placeholder="23"/>
                    </div>
                    <div class="w-[40%] h-[45px] flex items-center gap-6">
                        <InputNumber labelValue="قیمت تخمینی" id="price" placeholder="23" />
                    </div>
                </div>
                <div class="w-[260px]">
                ...
                <div class="w-full flex flex-col gap-10">
                    <div class="w-full flex items-center gap-6" >
                        <InputText class="w-[80%] align-start" style="width: 80%;" placeholder="متن نمونه" @keyup="changeQuestionName(index)" v-model="values.question" />
                ...
                <!-- <div class="flex flex-col gap-10 " v-if="questionNumber > 1" v-for="value in questionNumber-1" :key="value">
                    <div class="w-full flex items-center gap-6 " >
                        <InputText class="w-[80%] align-start" style="width: 80%;" placeholder="متن نمونه" @keyup="changeQuestionName(value)" v-model="values[value].question" />
                        <button @click="deleteOneQuestion(value)" class="w-[20px] h-[20px] flex items-center justify-center rounded-sm bg-[#F35242]/10 text-[#D02121] font-bold text-sm text-center leading-[normal]">
                            ✕  
                        </button>
                        {{ value }}
                    </div>
                    <div class="w-[80%] h-full align-start" >
                        <InputTextArea class="h-36" @keyup="changeAnswer(value)" placeholder="متن نمونه" v-model="values[value].answer" />
                    </div>
                </div> -->
                <button class="btn-primary bg-[#F2F5F7] px-5 text-[#488CDA]" @click="addQuestion">
