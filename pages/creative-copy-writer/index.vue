<template>
  <div class="w-full h-auto rounded-lg overflow-hidden text-sm">
    <div class="w-full h-full text-base-400 text-sm flex flex-col items-center justify-between p-7">
      <form @submit.prevent="creative_copy_writer_request()" class="w-full mb-4 flex items-center justify-center flex-col">
        <input type="text" placeholder="درج کلمه کلیدی" name="" id=""
      ...
      <div class="w-full h-auto mb-4 bg-white py-4 rounded border border-base-400 flex items-center justify-center">
        <div class="w-[20%] h-full text-base-content flex items-start justify-center flex-col text-lg">
          <p class="mt-16 w-full border-r-2 border-dimound px-2">
            {{ config.by_route(`${current_page}/categories/comparision`) }}
          </p>
          <button class="w-40 h-10 rounded-lg mr-2 text-sm mt-5 btn-secondary z-50">
            ...
            {{ config.by_route(`${current_page}/buttons/copy-subjects`) }}
          </button>
        </div>
      ...
      <!-- border2 -->
      <div class="w-full h-44 mb-4 bg-base-200 rounded border border-base-400 flex flex-col items-center justify-center">
...
const current_page = "pages/bulk-copy-writer";
