  ...
  <div class="w-full h-full">
    <div class="w-full h-1/2 items-center flex flex-row">
      <div class="w-1/2 h-full flex flex-col px-4 py-8 gap-8">
        <h2 class="border-r-2 pr-2 border-r-primary">افزودن صفحات تجاری</h2>
        <p class="pr-3">
          صفحات تجاری احتمالا مهم‌ترین صفحات وب‌سایت شما هستند که بخش زیادی از درآمد
          وب‌سایت را تولید می‌کنند. با درج آدرس‌شان در کادر زیر، گزارش این صفحات منتخب را
          دریافت می‌کنید، قابلیتی که تنها در سگمنتو تجربه خواهید کرد. در صفحه ابزار مربوط
          به صفحات تجاری امکان اضافه کردن کلمه کلیدی برای صفحات تجاری مسیر است.
        </p>
      </div>
      <div class="w-px bg-base-400 h-[90%]"></div>
      <div class="w-1/2 h-full flex flex-col items-center justify-center px-4 py-8">
        <img src="/images/MoneyPageStep.png" class="h-[150px]" alt="site png" />
      </div>
    </div>
    <div class="w-full h-1/2 flex flex-row justify-center items-center p-4 gap-4">
      <div class="gap-1 flex w-1/2 flex-col border h-full overflow-hidden" dir="ltr">
        ...
        <div class="w-full flex flex-col overflow-y-auto bg-white h-5/6">
          ...
          >
            <InputCheckbox
              v-model="modelValue.pages[index].is_money_page"
              :id="index"
              dir="ltr"
            >
              <span
