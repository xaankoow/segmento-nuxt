<template>
  ...
  >
    ...
    <div class="flex flex-col gap-12 items-center w-full pb-8">
      <!-- header and title -->
      <div
        class="w-11/12 flex flex-row items-center justify-center text-xl h-24"
        :class="plan_color()"
      >
        {{ plan.package().title }}
      </div>
      ...
      <div class="w-11/12 flex flex-col gap-4">
        <div
          class="flex flex-row items-center justify-between p-2 border text-base-500"
          v-for="pln in plan.plans()"
          :key="pln.id"
        >
          <!-- title -->
          <div class="flex flex-row items-center gap-2">
            <input
              ...
              class="w-4 h-4"
              name="radio"
              @change="plan.select_plan(pln.id)"
            />
            <span>{{ pln.title }}</span>
          </div>
          <!-- discount title -->
    ...
    <div class="flex flex-col items-center mb-2 w-11/12 gap-12">
      ...
      <div class="custom_input_box my-2">
        ...
        <label
          ...
          :class="discountBox.transitionStyle(discount, 'text-base-content')"
        >
          {{ config.by_route("components/plan/discount") }}</label
        >
        <button
          @click="discount_result = plan.check_discount(discount)"
...
function plan_color() {
  return `text-${props.plan.package().color} border-b border-b-${
    props.plan.package().color
  }`;
}
</script>